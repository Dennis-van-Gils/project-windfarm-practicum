...
/**************************************************************************/
/*!
    @brief Updates the shunt calibration value to the INA228 register, based
    on members `_shunt_res`, `_current_lsb` and `_adc_range`.
*/
/**************************************************************************/
...
void Adafruit_INA228::setADCRange(uint8_t adc_range) {
  ...
  _adc_range = (Config->read() >> 4) & 1; // Check truly set value
  _updateShuntCalRegister();
}
